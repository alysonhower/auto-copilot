...
def parse_time(time_str: str) -> time:
    ...
    try:
        ...
        raise ValueError(f"Invalid time format '{time_str}'. Use HH:MM (e.g., 07:00)") from e


def is_within_schedule(start_time: time, stop_time: time, now: Optional[time] = None) -> bool:
    """
...
async def wait_until_schedule_starts(start_time: time, stop_time: time) -> None:
    ...
    click.echo(f"⏸️  Outside schedule ({start_time.strftime('%H:%M')} - {stop_time.strftime('%H:%M')})")
    click.echo(f"⏳ Waiting until {start_time.strftime('%H:%M')} to resume...")
    
    while not is_within_schedule(start_time, stop_time):
        await asyncio.sleep(CHECK_INTERVAL)
    
    click.echo(f"▶️  Schedule window opened. Resuming processing...")
    ...
    while True:
        try:
            ...
            if max_retries is not None and attempt >= max_retries:
                click.echo(f"❌ Max retries ({max_retries}) reached. Giving up.", err=True)
                raise
            
            # Check if outside schedule (should wait for schedule instead of retry)
            if start_time and stop_time and not is_within_schedule(start_time, stop_time):
                click.echo(f"⏸️  Outside schedule. Will retry when schedule resumes.", err=True)
